...
<head>
    ...
    <!-- Google font -->
    <?php /* hellofriend: Inter is self-hosted in hf-admin.css, no Google request needed */ ?>
...
<script>/* hellofriend: native scrolling instead of NiceScroll (it made scrolling slow) */(function($){if(!$||!$.fn){return;}var r=document.createElement('div'),n={rail:[r],railh:[r],resize:function(){return n;},remove:function(){return n;},show:function(){return n;},hide:function(){return n;},doScrollTop:function(){return n;},eq:function(){return n;}};$.fn.niceScroll=function(){return n;};$.fn.getNiceScroll=function(){return n;};})(window.jQuery);</script>

<script>/* hellofriend: heavy libraries load only on the pages that use them. When the admin moves to
 such a page with AJAX, hfEnsure() loads what it needs first, then shows the page. */
window.hfAdminLibs = {
  apex: ['<?php echo Wo_LoadAdminLink('vendors/charts/apex/apexcharts.min.js'); ?>'],
  datatables: ['<?php echo Wo_LoadAdminLink('vendors/dataTable/datatables.min.js'); ?>'],
  tinymce: ['<?php echo Wo_LoadAdminLink('vendors/tinymce/js/tinymce/tinymce.min.js'); ?>'],
  codemirror: ['<?php echo Wo_LoadAdminLink('vendors/codemirror-5.30.0/lib/codemirror.js'); ?>', '<?php echo Wo_LoadAdminLink('vendors/codemirror-5.30.0/mode/css/css.js'); ?>', '<?php echo Wo_LoadAdminLink('vendors/codemirror-5.30.0/mode/javascript/javascript.js'); ?>']
};
window.hfAdminPageLibs = { 'dashboard': ['apex'], '': ['apex'], 'pro-payments': ['apex'], 'edit-lang': ['datatables'], 'manage-announcements': ['tinymce'], 'manage_emails': ['tinymce'], 'edit-terms-pages': ['tinymce'], 'custom-code': ['codemirror'] };
window.hfLoaded = {};
window.hfEnsure = function (ajaxUrl, done) {
  var m = /[?&]path=([^&]*)/.exec(ajaxUrl || ''), page = m ? decodeURIComponent(m[1]) : '';
  var urls = [];
  (window.hfAdminPageLibs[page] || []).forEach(function (lib) { if (!window.hfLoaded[lib]) { window.hfLoaded[lib] = 1; urls = urls.concat(window.hfAdminLibs[lib]); } });
  (function next() {
    if (!urls.length) { done(); return; }
    var sc = document.createElement('script'); sc.src = urls.shift(); sc.onload = next; sc.onerror = next; document.head.appendChild(sc);
  })();
};
</script>
    <?php $hf_libs_page = array('dashboard' => 'apex', '' => 'apex', 'pro-payments' => 'apex', 'edit-lang' => 'datatables', 'manage-announcements' => 'tinymce', 'manage_emails' => 'tinymce', 'edit-terms-pages' => 'tinymce', 'custom-code' => 'codemirror'); $hf_lib = isset($hf_libs_page[$page]) ? $hf_libs_page[$page] : ''; ?>
    <!-- Apex chart (only where used) -->
    <?php if ($hf_lib == 'apex') { ?><script src="<?php echo(Wo_LoadAdminLink('vendors/charts/apex/apexcharts.min.js')) ?>"></script><script>window.hfLoaded.apex = 1;</script><?php } ?>

    <!-- Daterangepicker -->
    <script src="<?php echo(Wo_LoadAdminLink('vendors/datepicker/daterangepicker.js')) ?>"></script>

    <!-- DataTable (only where used) -->
    <?php if ($hf_lib == 'datatables') { ?><script src="<?php echo(Wo_LoadAdminLink('vendors/dataTable/datatables.min.js')) ?>"></script><script>window.hfLoaded.datatables = 1;</script><?php } ?>

    <!-- Dashboard scripts -->
    <script src="<?php echo(Wo_LoadAdminLink('assets/js/examples/pages/dashboard.js')) ?>"></script>
    <?php /* hellofriend: chart.min.js removed, no admin page uses Chart.js */ ?>
...
<link href="https://fonts.googleapis.com/icon?family=Material+Icons&display=block" rel="stylesheet" type="text/css" media="print" onload="this.media='all'"><?php /* hellofriend: backup only, loads without blocking the page */ ?>
<?php //if ($page == 'create-article' || $page == 'edit-article' || $page == 'manage-announcements' || $page == 'newsletters') { ?>
<?php if ($hf_lib == 'tinymce') { ?><script src="<?php echo Wo_LoadAdminLink('vendors/tinymce/js/tinymce/tinymce.min.js'); ?>"></script><script>window.hfLoaded.tinymce = 1;</script><?php } ?>
...
<?php if ($hf_lib == 'codemirror') { ?><script src="<?php echo Wo_LoadAdminLink('vendors/codemirror-5.30.0/lib/codemirror.js'); ?>"></script>
<script src="<?php echo Wo_LoadAdminLink('vendors/codemirror-5.30.0/mode/css/css.js'); ?>"></script>
<script src="<?php echo Wo_LoadAdminLink('vendors/codemirror-5.30.0/mode/javascript/javascript.js'); ?>"></script><script>window.hfLoaded.codemirror = 1;</script><?php } ?>
...
<?php //} ?>
    ...
    <!-- hellofriend admin skin -->
    <link rel="stylesheet" href="<?php echo(Wo_LoadAdminLink('assets/css/hf-admin.css')) ?>?hf=3.0.0" type="text/css">
    <link rel="stylesheet" href="<?php echo(Wo_LoadAdminLink('assets/css/hf-admin-icons.css')) ?>?hf=3.0.0" type="text/css">
</head>
        ...
        $(document).on('click', 'a[data-ajax]', function(e) {
            ...
            if (($(this)[0].hasAttribute("data-sent") && $(this).attr('data-sent') == '0') || !$(this)[0].hasAttribute("data-sent")) {
                ...
                $.post(Wo_Ajax_Requests_File_load() + url, {url:url}, function (data) {
                    ...
                    hfEnsure(url, function () { $('.content').html(data); });
                    setTimeout(function () {
            ...
            <div class="header-left">
                ...
                <div class="header-logo">
                    <a href="<?php echo $wo['config']['site_url'] ?>">
                        ...
                        <img class="hfa-logo-dark" src="<?php echo $wo['config']['theme_url'];?>/img/logo-light.png" alt="<?php echo htmlspecialchars($wo['config']['siteName']); ?>"><span class="hfa-admin-tag">Admin</span>
                    </a>
                </div>
            </div>
            ...
            <div class="navigation-menu-body">
                <ul>
                   ...
                   <!--  <li <?php echo ($page == 'manage-updates') ? 'class="active"' : ''; ?>>
                    ...
                    <?php /* hellofriend: WoWonder changelog link removed */ ?>
                    <?php /* hellofriend: WoWonder FAQ link removed */ ?>
                    <?php /* hellofriend: "Powered by WoWonder" box removed */ ?>
                </ul>
            </div>
        </div>
        <!-- end::navigation -->
...
<script src="<?php echo(Wo_LoadAdminLink('vendors/select2/js/select2.min.js')) ?>"></script>
    ...
    <script src="<?php echo(Wo_LoadAdminLink('assets/js/hf-admin.js')) ?>?hf=3.0.0"></script>
