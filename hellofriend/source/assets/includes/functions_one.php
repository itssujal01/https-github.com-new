...
// hellofriend: the language table (2,500+ rows) was read from MySQL on every request,
// including the notification poll. Keep a copy on disk for a few minutes instead.
function hf_lang_cache_file($name)
{
    return dirname(__DIR__, 2) . '/cache/hf-lang/' . preg_replace('/[^a-z0-9_]/i', '', $name) . '.php';
}
function hf_lang_cache_get($name)
{
    $file = hf_lang_cache_file($name);
...
function hf_lang_cache_put($name, $data)
{
    if (empty($data)) {
        return;
    }
    $dir = dirname(hf_lang_cache_file($name));
    if (!is_dir($dir)) {
        @mkdir($dir, 0755, true);
    }
    $tmp = hf_lang_cache_file($name) . '.' . getmypid() . '.tmp';
    if (@file_put_contents($tmp, '<?php return ' . var_export($data, true) . ';', LOCK_EX) !== false) {
        @rename($tmp, hf_lang_cache_file($name));
    }
}
function hf_lang_cache_clear()
{
    foreach ((array) glob(dirname(__DIR__, 2) . '/cache/hf-lang/*.php') as $file) {
        @unlink($file);
    }
}
function Wo_LangsFromDB($lang = 'english')
{
    ...
    $cached = hf_lang_cache_get('lang_' . $lang);
    if ($cached !== null) {
        return $cached;
    }
    ...
    hf_lang_cache_put('lang_' . $lang, $data);
    return $data;
}
...
function Wo_LangsNamesFromDB($lang = 'english')
{
    global $sqlConnect, $wo;
    $data = array();
    $cached = hf_lang_cache_get('names');
    if ($cached !== null) {
        return $cached;
    }
...
// hellofriend: a feed page asked for the same people again and again (about 10 queries each time).
// While a page is being drawn, remember each answer for the rest of that request.
function hf_user_memo_enabled()
{
    static $on = null;
    if ($on === null) {
        $script = basename(isset($_SERVER['SCRIPT_NAME']) ? $_SERVER['SCRIPT_NAME'] : '');
        $f = isset($_GET['f']) ? $_GET['f'] : '';
        $on = (php_sapi_name() !== 'cli') && ($script === 'index.php' || $script === 'ajax_loading.php'
            || ($script === 'requests.php' && in_array($f, array('load_posts', 'posts', 'load-more-posts', 'get_more_posts', 'update_data'))));
    }
    return $on;
}
function Wo_UserData($user_id, $password = true)
{
    global $wo;
    static $memo = array();
    if (!hf_user_memo_enabled()) {
        return hf_UserDataFresh($user_id, $password);
    }
    $viewer = (!empty($wo['loggedin']) && !empty($wo['user']['user_id'])) ? $wo['user']['user_id'] : 0;
    $key = $user_id . '|' . ($password ? 1 : 0) . '|' . $viewer;
    if (!array_key_exists($key, $memo)) {
        $memo[$key] = hf_UserDataFresh($user_id, $password);
    }
    return $memo[$key];
}
function hf_UserDataFresh($user_id, $password = true)
{
    global $wo, $sqlConnect, $cache, $db;
    if (empty($user_id) || !is_numeric($user_id) || $user_id < 0) {
        return false;
