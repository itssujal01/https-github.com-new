...
// hellofriend: while `hf_free_pro` is on (Admin > settings, or Wo_Config), every signed-in member
// gets the biggest Pro package for the current request only. Nothing is written to the database,
// so turning the switch off later puts the normal paid plans back as they were.
$wo['hf_free_pro'] = (!isset($wo['config']['hf_free_pro']) || $wo['config']['hf_free_pro'] == 1) && $wo['config']['pro'] == 1;
if ($wo['hf_free_pro'] && $wo['loggedin'] && !empty($wo['user']) && !empty($wo['pro_packages'])) {
    $hf_best = null;
    foreach ($wo['pro_packages'] as $hf_id => $hf_pkg) {
        if (isset($hf_pkg['status']) && $hf_pkg['status'] != 1) {
            continue;
        }
        $hf_score = (int) $hf_pkg['posts_promotion'] * 1000000 + (int) $hf_pkg['pages_promotion'] * 1000 + (int) ($hf_pkg['max_upload'] / 1000000);
        if ($hf_best === null || $hf_score > $hf_best[0]) {
            $hf_best = array($hf_score, $hf_id);
        }
    }
    if ($hf_best !== null && (empty($wo['user']['is_pro']) || empty($wo['pro_packages'][$wo['user']['pro_type']]))) {
        $wo['user']['is_pro'] = 1;
        $wo['user']['pro_type'] = $hf_best[1];
        $wo['user']['pro_time'] = time();
        $wo['user']['hf_free_pro'] = 1;
    }
}
if ($wo['hf_free_pro'] && isset($_GET['link1']) && $_GET['link1'] == 'go-pro' && basename($_SERVER['SCRIPT_NAME']) == 'index.php') {
    header('Location: ' . $wo['config']['site_url']);
    exit();
}
// hellofriend: one-time database/file upgrade after new files are uploaded
require_once(__DIR__ . '/hf_upgrade.php');
hf_upgrade_run();
