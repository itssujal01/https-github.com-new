...
$(function () {
  ...
  if(current_width > 900 || api) {
    // hellofriend: run the scroll work at most once per frame (it measured the whole page on every scroll event)
    var hfScrollQueued = false;
    $(window).on('scroll', function () {
      if (hfScrollQueued) { return; }
      hfScrollQueued = true;
      window.requestAnimationFrame(function () { hfScrollQueued = false; hfOnScroll(); });
    });
    function hfOnScroll() {
...
function Wo_UpdateLastSeen() {
  if (document.hidden) { setTimeout(Wo_UpdateLastSeen, 60000); return; }
...
// hellofriend: poll less when nobody is looking. The old loop hit the server every 5s from
// every open tab, even hidden ones, which is what made shared hosting slow for everyone.
var hfLastActive = Date.now();
['pointerdown', 'keydown', 'scroll', 'touchstart'].forEach(function (ev) {
  window.addEventListener(ev, function () { hfLastActive = Date.now(); }, { passive: true });
});
function hfPollDelay() {
  if (document.hidden) { return 60000; }
  var idle = Date.now() - hfLastActive;
  if (idle > 10 * 60000) { return 30000; }
  if (idle > 2 * 60000) { return 15000; }
  return 7000;
}
document.addEventListener('visibilitychange', function () {
  if (!document.hidden && typeof intervalUpdates !== 'undefined' && node_socket_flow == "0") {
    clearTimeout(intervalUpdates);
    intervalUpdates = setTimeout(Wo_intervalUpdates, 300);
  }
});
// Notifications & follow requests updates
function Wo_intervalUpdates(force_update = 0, loop = 0) {
    if (document.hidden && force_update == 0) {
      clearTimeout(intervalUpdates);
      intervalUpdates = setTimeout(Wo_intervalUpdates, hfPollDelay());
      return;
    }
...
// hellofriend: @mention boxes start working when they are first focused. Setting up
// every comment box of every post at load time added page-wide listeners per box,
// which made every tap slower as the feed grew.
(function ($) {
  var orig = $.fn.triggeredAutocomplete;
  if (!orig) { return; }
  $.fn.triggeredAutocomplete = function (opts) {
    if (typeof opts !== 'object' || opts === null) { return orig.apply(this, arguments); }
    return this.each(function () {
      var $el = $(this);
      if ($el.data('ui-triggeredAutocomplete') || $el.data('uiTriggeredAutocomplete')) { return; }
      if (document.activeElement === this) { orig.call($el, opts); return; }
      $el.off('focusin.hfta').one('focusin.hfta', function () {
        if (!$el.data('ui-triggeredAutocomplete') && !$el.data('uiTriggeredAutocomplete')) { orig.call($el, opts); }
      });
    });
  };
})(jQuery);
